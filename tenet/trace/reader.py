...
class TraceReader(object):
    ...
    def find_next_breakpoint_index(self, start_idx, trace_breakpoint_addresses):
        """
        Return the next timestamp index that executes an address present in the breakpoint set.

        Args:
            start_idx (int): The index to start searching from (exclusive).
            trace_breakpoint_addresses (set): A set of breakpoint addresses in the trace's native address space.

        Returns:
            int: The index of the next breakpoint hit, or -1 if none found before the end of the trace.
        """
        if not trace_breakpoint_addresses:
            logger.debug("find_next_breakpoint_index called with empty breakpoint set.")
            return -1 # No breakpoints to find

        idx = max(0, start_idx + 1) # Start searching from the next index
        logger.debug(f"Searching for next breakpoint starting from index {idx}...")

        # TODO: Optimize this search. Linear scan can be slow for large traces.
        # Potential optimizations:
        # 1. Use segment data: Check if any breakpoints fall within a segment's IP range before iterating.
        # 2. Precompute breakpoint locations within segments if TraceFile format allows.
        # 3. Use binary search if execution IPs within segments are sorted (they usually are).
        
        while idx < self.trace.length:
            # Using internal trace.get_ip for potentially slightly better performance in a loop
            # compared to self.get_ip which might go through register cache logic.
            current_ip = self.trace.get_ip(idx)
            if current_ip in trace_breakpoint_addresses:
                logger.debug(f"Found breakpoint hit at index {idx} (IP: 0x{current_ip:X})")
                return idx # Found a breakpoint hit
            
            # Simple linear scan for now
            idx += 1

        # Reached end of trace without finding a breakpoint
        logger.debug("Reached end of trace without finding a breakpoint.")
        return -1

    def find_return_index(self, start_idx):
        """
        Find the index in the trace immediately after the current function returns.

        Args:
            start_idx (int): The index from which the 'step out' operation starts.

        Returns:
            int: The index after the function returns, or -1 if the return cannot be found
                 before the end of the trace or the function context is invalid.
        """
        if not self.dctx:
            logger.warning("Disassembler context (dctx) not available for step out analysis.")
            return -1
        
        # Check if dctx has the required methods before proceeding
        if not all(hasattr(self.dctx, attr) for attr in ['get_func', 'get_func_name', 'is_call_insn', 'is_ret_insn']):
             logger.error("Disassembler context (dctx) is missing required methods for step out analysis.")
             return -1
        ...
        try:
            start_func = self.dctx.get_func(start_rebased_ip)
             ...
             logger.error(f"Error getting function for address 0x{start_rebased_ip:X}: {e}")
             start_func = None # Ensure start_func is None if get_func fails

        if not start_func:
            logger.warning(f"Could not determine function context for address 0x{start_rebased_ip:X} at index {start_idx}.")
            ida_kernwin.info(f"Tenet: Cannot step out, function context unknown at 0x{start_rebased_ip:X}.")
            return -1
        
        start_func_ea = start_func.start_ea # Cache for performance
        end_func_ea = start_func.end_ea
        logger.debug(f"Starting step out from index {start_idx} within function {self.dctx.get_func_name(start_func_ea)} (0x{start_func_ea:X} - 0x{end_func_ea:X})")
        ...
        while idx < self.trace.length:
            ...
            # but is a necessary starting point. Consider re-evaluating function context if ip goes out of bounds?
            is_within_start_func = start_func_ea <= rebased_ip < end_func_ea

            # Check instruction type using disassembler context
            try:
                # Optimization: Decode instruction once if needed for both checks
                # For now, separate calls are okay.
                is_call = self.dctx.is_call_insn(rebased_ip)
                is_ret = self.dctx.is_ret_insn(rebased_ip)
            except Exception as e:
                # Handle cases where dctx methods might fail (e.g., address not decodable)
                logger.error(f"Error analyzing instruction at 0x{rebased_ip:X} (Index {idx}): {e}")
                idx += 1
                continue # Skip this instruction if analysis fails

            # Logic for call depth adjustment based on call/ret instructions
            if is_call and is_within_start_func:
                # If we are still within the original function and encounter a call, increase depth
                call_depth += 1
                # logger.debug(f"Index {idx}: Call detected within start func at 0x{rebased_ip:X}, depth -> {call_depth}")
            elif is_ret:
                if call_depth > 0:
                    # If it's a return from a nested call (could be inside or outside start_func bounds theoretically)
                    call_depth -= 1
                    # logger.debug(f"Index {idx}: Return detected from nested call at 0x{rebased_ip:X}, depth -> {call_depth}")
                elif call_depth == 0:
                    # This is the return instruction that balances the initial call depth (it *should* be the return from start_func)
                    logger.debug(f"Index {idx}: Potential return from start func detected at 0x{rebased_ip:X}. Target index is {idx + 1}")
                    # We want the index *after* the return instruction executes
                    target_idx = idx + 1
                    if target_idx < self.trace.length:
                        return target_idx
                    else:
                        # Return is the last instruction, seek to it.
                        logger.debug("Return from start func is the last instruction in trace.")
                        return idx

            # Move to the next trace index
            idx += 1

        logger.warning("Reached end of trace without finding function return.")
        ida_kernwin.info("Tenet: Reached end of trace before function return.")
        return -1 # Return not found before end of trace
