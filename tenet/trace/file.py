...
try:
    # Import necessary Arch classes within the try block
    from tenet.util.log import pmsg
    from tenet.trace.arch import ArchAMD64, ArchX86, ArchAArch64 # Add ArchAArch64
...
#
    ...
    # Ensure all needed Arch classes are imported in standalone mode too
    from arch import ArchAMD64, ArchX86, ArchAArch64 # Add ArchAArch64
...
class TraceFile(object):
    ...
    def _select_arch(self, magic):
        """
        Select the trace CPU arch based on the given magic value from the .tt file header.
        """
        # Import Arch classes locally if needed, or ensure they are available via self.trace or similar context
        # For simplicity, assuming they are imported at the module level
        # Make sure logging is imported if not already
        import logging
        logger = logging.getLogger("Tenet.Trace.File") # Get a logger instance

        # We need access to the Arch classes themselves to compare MAGIC
        try:
             # Attempt import relative to current package structure if running as part of tenet
             from .arch import ArchAMD64, ArchX86, ArchAArch64
        except ImportError:
             # Fallback for standalone execution (assuming arch.py is in the same dir)
             from arch import ArchAMD64, ArchX86, ArchAArch64

        logger.info(f"Selecting architecture based on magic: 0x{magic:X}")

        if ArchAMD64.MAGIC == magic:
            self.arch = ArchAMD64()
            logger.info("TraceFile: Detected AMD64 architecture from magic.")
        elif ArchAArch64.MAGIC == magic:
             self.arch = ArchAArch64()
             logger.info("TraceFile: Detected AArch64 architecture from magic.")
        elif ArchX86.MAGIC == magic: # Explicitly check for x86 too
             self.arch = ArchX86()
             logger.info("TraceFile: Detected X86 architecture from magic.")
        else:
            # Fallback or raise error if magic is unknown
            logger.error(f"TraceFile: Unknown architecture magic 0x{magic:X}. TraceFile object might have incorrect architecture!")
            # Keep the arch object potentially passed during __init__ or default set there.
            # If self.arch wasn't set correctly before this call (e.g., if None was passed to __init__),
            # the default fallback in __init__ might have already set it incorrectly.
            # It's crucial that the caller (__init__ or TraceReader) provides the correct arch if possible.
            # We won't change self.arch here if it was already set, assuming the caller knew better.
            if not self.arch:
                 logger.warning("TraceFile: No architecture was set prior to _select_arch. Defaulting to ArchX86 due to unknown magic.")
                 self.arch = ArchX86() # Fallback if no arch was provided initially
            else:
                 logger.warning(f"TraceFile: Keeping pre-existing arch '{self.arch.__class__.__name__}' despite unknown magic.")
...
class TraceSegment(object):
    ...
    def from_lines(self, lines):
        ...
        # Skip the first line if it was parsed as a comment containing the base address
        start_line_index = 1 if self.id == 0 and self.trace.parsed_comment_line else 0
        self._process_lines(lines[start_line_index:])
    ...
    def _process_lines(self, lines):
        ...
        try:
            ...
            for line in lines:
                # Call _process_line, which returns True on success, False on failure
                success = self._process_line(line, relative_idx)
                if success:
                    relative_idx += 1 # Only increment if the line was fully processed successfully
                # else: if success is False, loop continues to next line, relative_idx is not incremented
    ...
    def _process_line(self, line, relative_idx):
        ...
        for item in delta:
            ...
            parts = item.split("=", 1)
            if len(parts) != 2:
                pmsg(f"Skipping malformed item '{item}' in line ~{self.base_idx+relative_idx:,}")
                continue # Skip malformed item but continue processing the line
            name, value = parts
            name = name.strip().upper() # Convert name to uppercase and strip whitespace

            try: # Add try-except around individual item processing

                # special compression of IP
                if name == IP:
                    ip = int(value, 16) # This might fail for "0x0SP=..."
                ...
                # handle memory r/w/rw access
                    ...
                    # Unknown key name is an error for the whole line? Maybe just skip?
                    pmsg(f"Skipping unknown key '{name}' in line ~{self.base_idx+relative_idx:,}")
                    # Continue processing other items in the line instead of failing the whole line
            except Exception as e:
                # Any other error during item processing (int conversion, unhexlify, etc.), just skip item
                pmsg(f"Failed processing item '{item}' in line ~{self.base_idx+relative_idx:,}: {e}")
                # Continue processing other items in the line instead of failing the whole line

        self._pack_registers(registers, relative_idx)

        return True # Return True only if all items were processed successfully
