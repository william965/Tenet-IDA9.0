...
class RegisterArea(QtWidgets.QAbstractScrollArea):
    ...
    def _init_reg_positions(self):
        ...
        name_x, y = int(self._reg_pos[0]), int(self._reg_pos[1])
        ...
        arrow_size = (int(value_size.height() * 0.70) & 0xFE) + 1  # 确保是奇数
        ...
        for reg_name in regs:
            ...
            prev_x = int(name_x + int(name_size.width()) + self._char_width * 0.5) # Reduced spacing
            prev_rect.moveCenter(name_rect.center())
            prev_rect.moveLeft(int(prev_x))

            value_x = int(prev_x + prev_rect.width() + self._char_width * 0.5) # Reduced spacing
            ...
            next_x = int(value_x + int(value_size.width()) + self._char_width * 0.5) # Reduced spacing
