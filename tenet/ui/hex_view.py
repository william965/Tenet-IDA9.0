...
class HexView(QtWidgets.QAbstractScrollArea):
    ...
    def __init__(self, controller, model, parent=None):
        ...
        font = QtGui.QFont("Courier", pointSize=normalize_font(12))
    ...
    def _refresh_painting_metrics(self):
        ...
        self._pos_hex = int(self._width_addr + self._char_width)
        self._width_hex = int(self._chars_in_line * self._char_width)

        # the x position and width of the auxillary region (right section of view)
        self._pos_aux = int(self._pos_hex + self._width_hex)
        self._width_aux = int((self.model.num_bytes_per_line * self._char_width) + self._char_width * 2)
    ...
    def full_size(self):
        ...
        height = int(height * self._char_height)
    ...
    def _paint_line(self, painter, line_idx):
        ...
        x, y = int(self._pos_hex), int((line_idx + 1) * self._char_height)
    ...
    def _paint_byte(self, painter, byte_idx, x, y):
        ...
        """
        self._paint_text(painter, byte_idx, 1, int(x), y)
        x += int((2 + 1) * self._char_width)
    ...
    def _paint_dword(self, painter, byte_idx, x, y):
        ...
        for i in range(backwards_idx + 4, backwards_idx, -1):
            self._paint_text(painter, i, 0, int(x), y)
            x += int(self._char_width * 2)
    ...
    def _paint_text(self, painter, byte_idx, padding, x, y):
        ...
        y_bg = int(y - self._char_descent)

        width = int(self._char_width * (len(text) + padding))
        height = int(self._char_height)
